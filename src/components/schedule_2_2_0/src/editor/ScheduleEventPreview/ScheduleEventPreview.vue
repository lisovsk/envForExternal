  ...
  >
    <template v-if="!invalid">
      ...
      <div class="schedule-event-preview__content">
        ...
        <div @click.stop="/**/">
          <or-checkbox
            @input="runAtActivationChanged"
            v-model="isRunAtActivationLocal"
            :disabled="editable"
          >Run the event on activation</or-checkbox>
        </div>
      </div>
    </template>
...
export default {
  ...
  methods: {
    ...
    runAtActivationChanged(newVal) {
      this.$emit("update:isRunAtActivation", newVal);
    }
  },
  computed: {
    endDateComp() {
