...
export default {
  ...
  methods: {
    ...
    finalEndDate(endExpressionDate, isEndTime) {
      let endInterval;
      let endDate = !isEndTime
        ? moment(`${endExpressionDate} 23:59:59`).format('YYYY-MM-DD HH:mm:ss')
        : undefined;
      if (this.state === 'month') {
        endInterval = moment(
          ...
          this.selectedDays.map(item => {
            ...
            if (this.startDate && finalStartDate) {
              if (item.isReccuring && item.expressions.length > 0) {
                ...
                const cronFirstLaunchTime = `${item.runAtTime[0].mm} ${item.runAtTime[0].HH} ${cronDayOfMonth} ${cronMonth} ${cronDayOfWeeks} ${cronYear}`;
                const cronLastLaunchTime = `${item.runAtTime[runAtTimeLength].mm} ${item.runAtTime[runAtTimeLength].HH} ${cronDayOfMonth} ${cronMonth} ${cronDayOfWeeks} ${cronYear}`;
