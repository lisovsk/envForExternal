  ...
  >
    <template v-if="!invalid">
      ...
      <div class="schedule-event-preview__content">
        ...
        <div @click.stop="/**/">
          <or-checkbox v-model="isRunAtActivationComp">Run the event on activation</or-checkbox>
        </div>
      </div>
    </template>
  ...
  computed: {
    endDateComp() {
      ...
      return !this.isEndDate
        ...
        : `Recurring till <span class="bold-text">${moment(this.endDate).format(
            "ll"
          )}</span>`;
    },
    startsAt() {
      ...
      const endDate = !this.isEndDate
        ? undefined
        : new Date(`${moment(this.endDate).format("YYYY-MM-DD")}T23:59`);
    ...
    isRunAtActivationComp: {
      get() {
        return this.isRunAtActivation;
      },
      set(newVal) {
        this.$emit("update-isRunAtActivation", this.index, newVal);
      }
    }
  }
};
