        ...
        <template scope="item">
          <div class="schedule__wr-event-preview" @click="doEditable(item.index)">
            <schedule-event-preview
              ...
              :end-date="item.item.scheduleEventData.endExpression.date"
              :is-end-date="item.item.scheduleEventData.isEndTime"
              ...
              :is-run-at-activation.sync="item.item.scheduleEventData.isRunAtActivation"
              :index="item.index"
              @do-editable="doEditable"
              @copy-event="copyEvent"
              @delete-event="deleteEvent"
              @update-isRunAtActivation="updateIsRunAtActivation"
              :readonly="readonly"
            ></schedule-event-preview>
          </div>
        </template>
...
export default {
  ...
  data() {
    return {
      ...
      dataStates: this.scheduleEvents.map(() => "saved"), //[],
      numOfTryEdit: null,
      runAtTime: null,
      calendarTimeZone: "",
      isOpenModal: false
  ...
  methods: {
    updateIsRunAtActivation(index, value) {
      this.scheduleEvents[index].scheduleEventData.isRunAtActivation = value;
    },
    ...
    listNewItemMethod() {
      console.log("defaultValues.weekly", defaultValues.weekly);
      return {
        scheduleEventData: {
          id: uuid.v4(),
          startExpression: {
    ...
    copyEvent(index) {
      ...
      copyOfEvent.scheduleEventData.eventName = `Copy of ${copyOfEvent.scheduleEventData.eventName}`.slice(
        0,
        140
      );
      copyOfEvent.scheduleEventData.color = getRandomColor();
    ...
    deleteСonfirmation() {
      ...
      this.editableEventNum = null;
      // this.dataStates[this.itemIndexForDelete] = "canceled";
      this.dataStates.splice(this.itemIndexForDelete, 1);
