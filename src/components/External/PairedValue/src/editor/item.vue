<template>
  <div class="variable" :style="{'border-color':($v.$error)?'#f95d5d':''}">
    ...
    <div class="variable__name">
     <or-text-expression :style="{'align-self':(variableIsCodeLocal)?'flex-start':'flex-end'}"
...
export default {
  ...
  computed: {
    ...
    variableValueLocal: {
      ...
      set(newValue) {
        newValue = this.valueTypeLocal == "number" ? newValue : newValue;
