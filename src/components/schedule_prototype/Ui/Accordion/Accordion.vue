<template>
  <div>
        ...
        <li class="accordion__item" :key="index" v-for="(slot, index) in slots">
          <div class="accordion__wr-content" @click="/*openItem(index)*/">
            <span
              ...
              :class="{
                                    'active' : true/*openedItemComp === index*/,
                                    'disabled': false/*openedItemComp && openedItemComp !== index*/,
                                    'invalid': false/*slotErrors[index] && openedItemComp === index*/
                                  }"
            >{{slot[0].data.attrs.titleItem}}</span>
            <div v-if="'openedItemComp === index'" class="accordion__slot">
              <slot :name="index"></slot>
            </div>
            <span
              v-if="false/*!openedItemComp*/"
              class="accordion__placeholder"
            >{{slot[0].data.attrs.placeholderItem}}</span>
          </div>
          <div
            v-if="false/*openedItemComp === index*/"
            class="accordion__close-item"
            @click="deleteItem"
          >
    ...
    <!-- <or-modal
      :contain-focus="false"
      ref="deleteSettingsInAccordionItemConfirmation"
      title="Delete settings"
    >
      Do you really want to delete
      <span class="accordion__bold">{{currentRecurringName}}</span> recurrent settings?
      <div slot="footer">
        <or-button
          color="red"
          @click="isDelete ? deleteSettingsInAccordionItemOrOpenOther() : deleteSettingsInAccordionItemOrOpenOther(true)"
        >Delete</or-button>
        <or-button
          color="primary"
          type="secondary"
          @click="closeModal('deleteSettingsInAccordionItemConfirmation')"
        >Cancel</or-button>
      </div>
    </or-modal>-->
  </div>
