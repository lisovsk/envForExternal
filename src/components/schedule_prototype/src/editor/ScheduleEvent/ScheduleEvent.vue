  ...
  <div class="schedule-event-scope">
    <div class="schedule-event">
      ...
      <accordion
        ...
        :invalid="false/*$v.validationCopyScheduleEventData.savedAccordionSlotName.$invalid && $v.validationCopyScheduleEventData.savedAccordionSlotName.$dirty*/"
      >
        <template slot="item1">
        ...
        <template slot="item2">
          <div placeholderItem="Set recurring weekly" titleItem="Weekly">
            <cron-generators-weekly
              ...
              :period-mode.sync="copyScheduleEventData.weekly.periodMode"
              :runAtTime="runAtTimeLocal"
              :readonly="readonly"
...
import { generateCrons } from "../CronGenerators/helpers.js";
  ...
  methods: {
    ...
    doExpressions() {
      if (this.copyScheduleEventData.isReccuring) {
        ...
        this.copyScheduleEventData.expressions = generateCrons(
          this.runAtTimeLocal,
          `${this.copyScheduleEventData.daily.cronExpressions} ${this.copyScheduleEventData.monthly.cronExpressions} ${this.copyScheduleEventData.weekly.cronExpressions} ${this.copyScheduleEventData.yearly.cronExpressions}`
        );
      }
    },
  ...
  watch: {
    ...
    copyScheduleEventData: {
      handler(newValue, oldValue) {
        if (newValue.id !== this.scheduleEventData.id) return;

        if (!_.isEqual(newValue, this.scheduleEventData)) {
          this.dataStateComp = "changed";
        } else if (this.dataStateComp === "changed") {
          this.dataStateComp = "canceled";
        }
      },
      deep: true
