...
export default {
  ...
  unregister(name) {
    if (name) {
      ...
      _.forEach(_.keys(this.registry), componentName =>
        this.unregister(componentName)
      );
    }
  },
  ...
  register({ name, template, code, style, args, noWrapStyle }) {
    return Promise.try(() => {
      const componentCode = _.startsWith(code, 'return')
        ? code
        : `return ${code}`;
      // eslint-disable-next-line no-new-func
      const descriptor = new Function(..._.keys(args), componentCode)(
        ..._.values(args)
      );
  ...
  registerInput(input) {
    return Promise.all([
      this._registerComponent(
        ...
        input.default || { template: `<h4>?DESIGN ${input.component}<h4>` }
      ),
      this._registerComponent(
        `edit-${input.component}`,
        input.default.components.editor || {
          template: `<h4>?EDIT ${input.component}<h4>`
        }
      )
    ]);
  },
  ...
  async _registerComponent(name, descriptor) {
    ...
    const styles = _.groupBy(
      descriptor.styles,
      style => (style.scoped ? 'local' : 'global')
    );
    const style = _.chain(styles.local)
      .map('content')
      .join('\n')
      .value();
    const noWrapStyle = _.chain(styles.global)
      .map('content')
      .join('\n')
      .value();
    const css = await this._compileStyles({
      wrapperClass: name,
      style,
      noWrapStyle
    });
  ...
  async _compileStyles({ style, wrapperClass, noWrapStyle }) {
    ...
    const styles = style
      ? `${noWrapStyle || ''} .${wrapperClass}{${style}}`
      : noWrapStyle;
